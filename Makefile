...
dotfiles: ## Installs the dotfiles.
	...
	for file in $(shell find $(CURDIR) -name ".*" -not -name ".gitignore" -not -name ".travis.yml" -not -name ".git" -not -name ".*.swp" -not -name ".gnupg" -not -name "rm-old-snaps.sh" -not -name "dpkg-filters"); do \
		f=$$(basename $$file); \
		ln -sfn $$file $(HOME)/$$f; \
	done; \
	# gpg --list-keys || true;
